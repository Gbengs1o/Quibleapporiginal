...
export default function GlitchLoader({ size = 100, message }: GlitchLoaderProps) {
    // Animations
    ...
    useEffect(() => {
        ...
        Animated.loop(
            Animated.sequence([
                Animated.timing(glitchAnim1, { toValue: 1, duration: 500, useNativeDriver: true, easing: Easing.linear }),
                Animated.timing(glitchAnim1, { toValue: 0, duration: 500, useNativeDriver: true, easing: Easing.linear }),
            ])
        ).start();
        ...
        Animated.loop(
            Animated.sequence([
                Animated.timing(glitchAnim2, { toValue: 1, duration: 600, useNativeDriver: true, easing: Easing.linear }),
                Animated.timing(glitchAnim2, { toValue: 0, duration: 600, useNativeDriver: true, easing: Easing.linear }),
            ])
        ).start();
        ...
        Animated.loop(
            Animated.sequence([
                Animated.timing(fillOpacity, { toValue: 0.8, duration: 800, useNativeDriver: true }),
                Animated.delay(200),
                Animated.timing(fillOpacity, { toValue: 0, duration: 600, useNativeDriver: true }),
                Animated.delay(400),
            ])
        ).start();
        ...
        // Grid rotation
        Animated.loop(
            Animated.timing(gridRotation, { toValue: 1, duration: 10000, useNativeDriver: true, easing: Easing.linear })
        ).start();
    }, []);

    const translateX1 = glitchAnim1.interpolate({
        inputRange: [0, 0.2, 0.4, 0.6, 0.8, 1],
        outputRange: [0, -3, 2, 0, -1, 0]
    });

    const translateY1 = glitchAnim1.interpolate({
        inputRange: [0, 0.2, 0.4, 0.6, 0.8, 1],
        outputRange: [0, 0, -1, 1, 0, 0]
    });

    const translateX2 = glitchAnim2.interpolate({
        inputRange: [0, 0.2, 0.4, 0.6, 0.8, 1],
        outputRange: [0, 2, -1, 0, 1, 0]
    });

    const gridSpin = gridRotation.interpolate({
        inputRange: [0, 1],
    ...
    return (
        <View style={[styles.container, { width: size * 2, height: size * 2 }]}>
            ...
            <Animated.View style={[styles.grid, { transform: [{ rotate: gridSpin }], width: size * 1.5, height: size * 1.5 }]} />

            {/* Layer 1: Navy Chassis */}
            <Animated.View style={[styles.layer, { transform: [{ translateX: translateX1 }, { translateY: translateY1 }] }]}>
                <Svg width={size} height={size * 0.85} viewBox="0 0 171 146">
                    <Path d={LOGO_PATH_1} stroke="#1F2050" strokeWidth={3} fill="none" />
                    <Path d={LOGO_PATH_2} stroke="#1F2050" strokeWidth={3} fill="none" />
                </Svg>
            </Animated.View>

            {/* Layer 2: Magenta Echo */}
            <Animated.View style={[styles.layer, { transform: [{ translateX: translateX2 }] }]}>
                <Svg width={size} height={size * 0.85} viewBox="0 0 171 146">
                    <Path d={LOGO_PATH_1} stroke="#ff0055" strokeWidth={2} fill="none" opacity={0.6} />
                    <Path d={LOGO_PATH_2} stroke="#ff0055" strokeWidth={2} fill="none" opacity={0.6} />
                </Svg>
            </Animated.View>

            {/* Layer 3: Orange Fill */}
            <Animated.View style={[styles.layer, { opacity: fillOpacity }]}>
                <Svg width={size} height={size * 0.85} viewBox="0 0 171 146">
                    <Path d={LOGO_PATH_1} fill="#F4821F" />
                    <Path d={LOGO_PATH_2} fill="#F4821F" />
                </Svg>
            </Animated.View>
        </View>
    );
