-- Harden auth checks in complete_delivery_job_v2.
-- Equality checks with NULL can become NULL and weaken IF conditions.
...
BEGIN
    ...
    IF v_req.user_id IS DISTINCT FROM auth.uid()
       AND v_req.rider_id IS DISTINCT FROM auth.uid() THEN
        RETURN jsonb_build_object('success', false, 'message', 'Not authorized');
    END IF;
