-- Migration: Route Delivery Fees to Vendor Wallets
-- When a customer orders from multiple vendors, each vendor receives
-- their delivery fee so they can independently pay their own rider.
-- The 10% platform fee applies ONLY to item prices, NOT delivery fees.

-- 1. Update place_order (Restaurant Orders)
-- Adds p_delivery_fee parameter; credits it to restaurant wallet
...
BEGIN
    ...
    VALUES (user_wallet_id, v_grand_total, 'debit', 'Order Payment', new_order_id::text);

    INSERT INTO public.transactions (wallet_id, amount, type, description, reference)
    VALUES (restaurant_wallet_id, v_restaurant_share, 'credit', 'Order Earnings + Delivery Fee', new_order_id::text);

    IF admin_wallet_id IS NOT NULL THEN
        INSERT INTO public.transactions (wallet_id, amount, type, description, reference)
        VALUES (admin_wallet_id, v_platform_fee, 'credit', 'Platform Fee', new_order_id::text);
    END IF;
...
-- 2. Update place_store_order (Store Orders)
-- Adds p_delivery_fee parameter; credits items + delivery fee to store
...
BEGIN
    ...
    VALUES (v_user_wallet_id, v_grand_total, 'debit',
        'Store Order #' || substring(new_order_id::text, 1, 8), '{}'::jsonb, new_order_id::text);
    ...
    VALUES (v_store_wallet_id, v_store_share, 'credit',
        'Order Payout + Delivery Fee #' || substring(new_order_id::text, 1, 8), '{}'::jsonb, new_order_id::text);
    ...
    IF admin_wallet_id IS NOT NULL THEN
        ...
        VALUES (admin_wallet_id, v_service_fee, 'credit',
            'Service Fee from Store Order', '{}'::jsonb, new_order_id::text);
    END IF;
