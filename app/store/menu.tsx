...
interface MenuItem {
    id: string;
    store_id: string; // Changed from restaurant_id
    name: string;
    description: string;
    category: string; // Changed from enum to string for flexibility or specific store categories
    price: number;
    image_url: string | null;
    options: string | null; // Changed from sides
    is_active: boolean;
...
export default function StoreInventoryScreen() {
    ...
    const filterItems = () => {
        ...
        if (searchQuery.trim()) {
            ...
            items = items.filter(item =>
                ...
                (item.options && item.options.toLowerCase().includes(query))
            );
        }
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled && result.assets[0].base64) {
            try {
                const fileName = `store-item-${storeId}-${Date.now()}.jpg`;
                const { data, error } = await supabase.storage
                    .from('restaurant-documents') // We can reuse the bucket or create a new one 'store-documents'
    ...
    return (
        <ThemedView style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <ThemedText type="title" style={[styles.title, { color: textColor }]}>Inventory Management</ThemedText>
                <View style={{ width: 28 }} />
            </View>
                ...
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { backgroundColor: modalBg }]}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <TouchableOpacity style={[styles.imagePicker, { backgroundColor: inputBg }]} onPress={pickImage}>
                                {formData.image_url ? (
                                    ...
                                    <View style={styles.imagePickerPlaceholder}>
                                        ...
                                        <ThemedText style={{ color: secondaryText, marginTop: 8 }}>Add Product Image</ThemedText>
                                    </View>
                                )}
                            </TouchableOpacity>
                            ...
                            {/* Options */}
                            <ThemedText style={[styles.formLabel, { color: textColor }]}>Versions/Options (optional)</ThemedText>
                            <TextInput
                                style={[styles.input, { backgroundColor: inputBg, color: textColor }]}
                            ...
                            {/* Stock */}
                            <ThemedText style={[styles.formLabel, { color: textColor, mt: 10 }]}>Stock Quantity</ThemedText>
                            <TextInput
                                style={[styles.input, { backgroundColor: inputBg, color: textColor }]}
                                value={formData.stock_quantity.toString()}
                                onChangeText={(text) => setFormData({ ...formData, stock_quantity: text.replace(/[^0-9]/g, '') })}
                                placeholder="0"
                                placeholderTextColor={secondaryText}
                            ...
                            <View style={styles.activeToggleRow}>
                                <View>
                                    <ThemedText style={[styles.formLabel, { color: textColor, marginBottom: 0 }]}>Show in Store</ThemedText>
                                    <ThemedText style={{ color: secondaryText, fontSize: 12 }}>Make this product visible to customers</ThemedText>
                                </View>
                                <Switch
