...
export default function StorePromotionsScreen() {
    ...
    const fetchStoreProfile = async () => {
        ...
        const rawProfile = (data?.[0] as Pick<StoreProfile, 'id' | 'name' | 'address'> | undefined) || null;
        const profile: StoreProfile | null = rawProfile
            ? {
                ...rawProfile,
    ...
    const fetchItemOptions = async (storeId: string) => {
        ...
        try {
            ...
            console.error('Error fetching store item options:', error);
            setItemOptions([]);
        } finally {
    ...
    const fetchStoreAndPromos = async (showLoader = false) => {
        if (showLoader) setLoading(true);
        try {
            const str = await fetchStoreProfile();
            if (!str?.id) {
                setPromotions([]);
                return;
            }
    ...
    const ensureStoreProfileLoaded = async () => {
        if (store?.id) return store;
        try {
            const profile = await fetchStoreProfile();
            if (!profile?.id) {
                Alert.alert('Profile Required', 'No store profile is linked to this account yet.');
                return null;
            }
            return profile;
        } catch (error) {
            console.error('Error loading store profile:', error);
            Alert.alert('Error', 'Could not load your store profile. Please try again.');
            return null;
        }
    };

    const handleOpenCreateModal = async () => {
        const profile = await ensureStoreProfileLoaded();
    ...
    const getDestinationLabel = (promo: any) => {
        ...
        if (destinationType === 'item') return 'Destination: Specific Product';
    ...
    const handleSubmit = async () => {
        ...
        const activeStore = store?.id ? store : await ensureStoreProfileLoaded();
        ...
        if (destinationType === 'item' && !trimmedDestinationValue) {
            Alert.alert('Destination Required', 'Please select the destination product for this promotion.');
            return;
        }
        ...
        if (destinationType === 'item' && trimmedDestinationValue && itemOptions.length > 0) {
            ...
            if (!isValidItem) {
                Alert.alert('Invalid Product', 'Selected product is invalid. Please choose from your active products.');
                return;
            }
        }
        ...
        Alert.alert(
            ...
            [
                ...
                { text: 'Confirm', onPress: () => processPromotion(activeStore) },
            ]
        );
    };
    const processPromotion = async (activeStore: StoreProfile) => {
        ...
        try {
            ...
            const { data, error } = await supabase.rpc('create_promotion', {
                p_vendor_id: activeStore.id,
                p_vendor_type: 'store', // Changed to store
                p_title: title,
    ...
    return (
        <ThemedView style={styles.container}>
            <View style={styles.header}>
                ...
                <ThemedText type="title">Store Promotions</ThemedText>
                <TouchableOpacity
                    onPress={handleOpenCreateModal}
                    disabled={!store?.id}
                    style={!store?.id ? { opacity: 0.5 } : undefined}
                >
                ...
                ListEmptyComponent={
                    <View style={styles.emptyState}>
                        ...
                        <ThemedText style={styles.placeholderText}>
                            Boost your shop's visibility! Create your first promotion to appear on the homepage.
                        </ThemedText>
                        <TouchableOpacity
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <View style={styles.formGroup}>
                                ...
                                <TextInput
                                    ...
                                    placeholder="e.g. New Seasonal Collection"
                            ...
                            <View style={styles.formGroup}>
                                ...
                                {destinationType === 'item' && (
                                    <View style={[styles.destinationPicker, { backgroundColor: inputBg }]}>
                                        {loadingItemOptions ? (
                                            ...
                                            <ThemedText style={styles.destinationHint}>No active products found. Add and activate products first.</ThemedText>
                                        ) : (
                                ...
                                {destinationType === 'custom_url' && (
                                    <TextInput
                                        style={[styles.input, { backgroundColor: inputBg, color: iconColor }]}
                                        placeholder="https://example.com or /store-profile/id"
                                        placeholderTextColor="#888"
