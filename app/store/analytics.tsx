...
export default function AnalyticsScreen() {
    ...
    const [data, setData] = useState<AnalyticsData>({
        ...
        avgRating: 0,
        totalReviews: 0,
        ratingDistribution: { labels: [], datasets: [{ data: [0] }] },
    });
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            // Optimize labels: Show range 8-22, but only label even hours to prevent overlap
            for (let i = 8; i <= 22; i++) {
                // If it's an even hour (8, 10, 12...), show label. Otherwise empty string.
                const label = i % 2 === 0
                    ? (i > 12 ? `${i - 12}P` : (i === 12 ? '12P' : `${i}A`))
                    : '';
            ...
            setData({
                ...
                ratingDistribution: {
                    labels: ['5★', '4★', '3★', '2★', '1★'],
                    datasets: [{ data: [dist[5], dist[4], dist[3], dist[2], dist[1]] }]
                },
            });
            ...
            >
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.statsRow}>
                    ...
                    <View style={[styles.statCard, { backgroundColor: '#F1C40F' }]}>
                        ...
                        <ThemedText style={styles.statValue}>{data.avgRating.toFixed(1)}</ThemedText>
                        <ThemedText style={styles.statLabel}>Avg Rating ({data.totalReviews})</ThemedText>
                    </View>
                </ScrollView>
                ...
                <View style={[styles.card, { backgroundColor: chartBg }]}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <LineChart
                            ...
                            width={Math.max(width - 60, data.weeklyRevenue.labels.length * 50)} // Dynamic width
                            height={180}
                            yAxisLabel="₦"
                            yAxisSuffix=""
                ...
                {/* Review Distribution */}
                <View style={[styles.card, { backgroundColor: chartBg }]}>
                    <ThemedText style={styles.cardTitle}>⭐ Customer Satisfaction</ThemedText>
                    <ThemedText style={styles.cardSubtitle}>Rating distribution</ThemedText>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <BarChart
                            data={data.ratingDistribution}
                            width={Math.max(width - 60, 300)} // Ensure minimum width
                            height={180}
                            yAxisLabel=""
                            yAxisSuffix=""
                            chartConfig={{ ...chartConfig, color: () => '#F1C40F' }}
                            showValuesOnTopOfBars={true} // Add back values for ratings
                            fromZero
                            style={{ marginTop: 10, borderRadius: 8 }}
                        />
                    </ScrollView>
                ...
                <View style={[styles.card, { backgroundColor: chartBg }]}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <BarChart
                            data={data.peakTimes}
                            width={Math.max(width - 60, data.peakTimes.labels.length * 40)} // Wide chart for scrolling
                            height={200}
                            yAxisLabel=""
                            yAxisSuffix=""
                            chartConfig={chartConfig}
                            // showValuesOnTopOfBars={false} // Removed to declutter 0s
                            fromZero
                            segments={3} // Better y-axis for small numbers
                            style={{ marginTop: 10, borderRadius: 8 }}
                        />
                    </ScrollView>
                </View>
                ...
                <View style={[styles.card, { backgroundColor: chartBg }]}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <BarChart
                            data={data.dayOfWeek}
                            width={Math.max(width - 60, data.dayOfWeek.labels.length * 50)}
                            height={180}
                            yAxisLabel=""
                            yAxisSuffix=""
