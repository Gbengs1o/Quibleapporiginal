...
export default function RiderWallet() {
    ...
    const { riderWallet, transactions, isLoading, refreshWallet, transferFunds, requestPayout, activateWallet, resolveRecipient } = useWallet();
    ...
    useEffect(() => {
        activateWallet('rider'); // Enforce rider context
        refreshWallet();
    }, []);
    ...
    const balanceFontSize = getBalanceFontSize(riderWallet?.balance || 0);
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    <>
                        ...
                        {user && (
                            <View style={[styles.identityCard, { backgroundColor: cardBg }]}>
                                <View style={styles.identityRow}>
                                    <LinearGradient colors={['#1F2050', '#2a2b5e']} style={styles.restaurantLogo}>
                                        <Ionicons name="bicycle" size={24} color="#fff" />
                                    </LinearGradient>
                                    <View style={styles.identityInfo}>
                                        <ThemedText style={styles.restaurantName}>{user.user_metadata?.first_name || 'Rider'} {user.user_metadata?.last_name || ''}</ThemedText>
                                        <View style={styles.verifiedBadge}>
                                            <Ionicons name="checkmark-circle" size={14} color="#f27c22" />
                                            <ThemedText style={[styles.verifiedText, { color: '#f27c22' }]}>Rider Account</ThemedText>
                                        </View>
                                    </View>
                                </View>
                            </View>
                        ...
                        {/* Main Balance Card */}
                        <LinearGradient
                            colors={['#1F2050', '#2a2b5e', '#3b3c7a']} // Different gradient for rider
                            start={{ x: 0, y: 0 }}
                            end={{ x: 1, y: 1 }}
                        ...
                        >
                            ...
                            <View style={styles.balanceSection}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                                    ...
                                    >
                                        {showBalance ? `₦${(riderWallet?.balance || 0).toLocaleString('en-NG', { minimumFractionDigits: 2 })}` : '₦••••••••'}
                                    </ThemedText>
                                </View>
                            </View>
                            ...
                            <View style={styles.cardActions}>
                                {/* Fund button removed for Riders */}
                                <TouchableOpacity style={[styles.cardActionBtn, { flex: 1 }]} onPress={() => setShowWithdrawModal(true)}>
                                ...
                                <TouchableOpacity style={[styles.cardActionBtn, { flex: 1 }]} onPress={() => setShowTransferModal(true)}>
                                    <View style={styles.cardActionIcon}>
                                        <Ionicons name="swap-horizontal" size={22} color="#3b82f6" />
                                    </View>
                                    <ThemedText style={styles.cardActionText}>Transfer</ThemedText>
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.cardActionBtn, { flex: 1 }]} onPress={() => router.push('/history')}>
                                    <View style={styles.cardActionIcon}>
                        ...
                        <View style={styles.statsRow}>
                            <View style={[styles.statCard, { backgroundColor: cardBg }]}>
                                ...
                                <ThemedText style={[styles.statLabel, { color: secondaryText }]}>Total Earnings</ThemedText>
                            ...
                            <View style={[styles.statCard, { backgroundColor: cardBg }]}>
                                ...
                                <ThemedText style={[styles.statLabel, { color: secondaryText }]}>Total Withdrawn</ThemedText>
...
const styles = StyleSheet.create({
    ...
    cardActionBtn: { alignItems: 'center', gap: 8, flex: 1 },
    cardActionIcon: {
        width: 44,
        height: 44,
        borderRadius: 14,
